...
/** 통합 테스트 원형(부모) 부모이므로 테스트 자체는 실행되지 않도록 @Ignore를 붙여주었다. */
@Ignore
@Transactional
@SpringBootTest
...
public class IntegrationTest {
  ...
  static {
    rdbms =
        new DockerComposeContainer(new File("infra/test/docker-compose.yml"))
            ...
            // 해당 서비스에 연결하기 위한 호스트와 포트 번호가 testcontainers에 의해 동적으로 할당(포트 바인딩)된다. (예:
            // localhost:51212)
            .withExposedService(
                "local-db",
                3306,
  ...
  static class IntegrationTestInitializer
    ...
    public void initialize(ConfigurableApplicationContext applicationContext) {
      ...
      properties.put(
          "spring.datasource.url", "jdbc:mysql://" + rdbmsHost + ":" + rdbmsServicePort + "/score");
